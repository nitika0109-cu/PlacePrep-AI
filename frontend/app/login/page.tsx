...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Force a fresh page load after login so the dashboard
      // immediately reads the newly stored authentication token.
  ...
  const handleQuickDemo = async () => {
    ...
    try {
      ...
      // Force a fresh page load after demo login as well.
      window.location.replace(destination);
    } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          <div className="flex items-center justify-between text-xs pt-1">
            ...
            <button
              type="button"
              onClick={() =>
                alert('Password reset link sent to your college email.')
              }
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full py-2 rounded-xl bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/30 text-purple-300 text-xs font-medium transition flex items-center justify-center gap-1.5 disabled:opacity-50"
          >
